...
func TestEstimateMessagesTokens(t *testing.T) {
	...
	t.Run("messages with content and tool_calls", func(t *testing.T) {
		msgs := []database.ChatMessage{
			{Content: "abcd", ToolCalls: "abcdefgh"}, // 1 + 2 = 3 tokens
		}
		result := estimateMessagesTokens(msgs)
		if result != 3 {
			t.Errorf("expected 3, got %d", result)
...
func TestShouldTriggerSummarization(t *testing.T) {
	...
	t.Run("budget becomes zero or negative returns false", func(t *testing.T) {
		// contextWindow=1000, maxTokens=800, margin=250 → budget=-50
	...
	t.Run("under budget returns false", func(t *testing.T) {
		// contextWindow=100000, maxTokens=4096, margin=25000 → budget=70904
		// 10 msgs of 100 chars = 250 tokens → well under budget
	...
	t.Run("over budget returns true", func(t *testing.T) {
		// contextWindow=8000, maxTokens=2000, margin=2000 → budget=4000
		// 50 msgs of 400 chars each = 50*100 = 5000 tokens → exceeds 4000
	...
	t.Run("existing summary tokens count toward estimate", func(t *testing.T) {
		// contextWindow=8000, maxTokens=2000, margin=2000 → budget=4000
		// 30 msgs of 400 chars = 30*100 = 3000 tokens
		// summary of 4400 chars = 1100 tokens → total 4100 > 4000
		p := makeProfile(8000, 2000)
	...
	t.Run("default maxTokens when zero", func(t *testing.T) {
		// maxTokens=0 → defaults to 4096
		// contextWindow=20000, maxTokens=4096(default), margin=5000 → budget=10904
		// 5 msgs of 40 chars = 50 tokens → under
		p := &profiles.Profile{
			Chat: profiles.ChatConfig{
				ContextWindow: 20000,
				MaxTokens:     0,
...
func TestBuildSummarizationUserPrompt(t *testing.T) {
	...
	t.Run("long content is truncated at 2000 chars", func(t *testing.T) {
		...
		// The prompt should not contain the full 3000-char content
