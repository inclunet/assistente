...
Focus and return: conversation links focus a matching chat tab or open one. Tasklist links and links for live terminal IDs focus a matching tab or open one; editor/{id} opens a new editor tab because editor tabs are not matched by that ID. Navigation changes the user's current view. The validated profile tab is only tab=voice. Caller-aware UI flows can return to the originating workspace surface after profile save/cancel, but this tool's backend event carries no caller context, so a direct profile edit opened here returns to the profiles list.
Supported forms:
- Conversations: assistente://conversation/{id}, assistente://conversation/new with optional message and title query parameters, and assistente://conversation/{id}/send?message=... with required message. Optional profile={slug} applies to new, open, and send; use ? for the first query parameter and & for later ones.
- Tab open: assistente://tasklist/{id}, assistente://editor/{id}, assistente://terminal/{id}.
- New tabs: assistente://tasklist/new and assistente://editor/new accept optional title; assistente://terminal/new accepts optional cmd and title; assistente://editor/open?file=... requires file and accepts optional title.
- Pages: assistente://navigate/{route}; use assistente://navigate/ for the empty route. The frontend accepts only: empty route, settings, settings/providers, settings/mcp, settings/skills, settings/channels, settings/contacts, settings/credentials, settings/allowlists, settings/network-allowlist, settings/path-allowlist, settings/appearance, settings/data, settings/restore-defaults, profiles, history, memories, tasklists, help, about, update. Settings tabs require the settings/ prefix.
