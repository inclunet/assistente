...
Risk: executing a link can interrupt the user's focus; new/edit/send links can create or change app state, conversation send transmits a message, and terminal cmd can run a command under the normal shell allowlist/confirmation policy. A deep link does not grant content access or bypass validation, authorization, tool policy, or confirmations. The Go tool requires a non-empty URI after trimming and checks the assistente:// prefix; the frontend parser rejects unsupported navigate routes and invalid required or validated parameter combinations. If unavailable, discover and load open_deep_link with tool_catalog when the profile permits on-demand tools.`
}
