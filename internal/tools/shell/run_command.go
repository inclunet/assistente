...
func (rc *RunCommand) Description() string {
	return `Executes a shell command in a PTY. Use when a task requires a process, such as building, testing, Git, or a CLI; for example {"command":"go test ./...","timeout_seconds":120}. Do not use when a dedicated file/search tool can perform the operation, or merely to list, create, interrupt, or close terminals—use terminal_session for that lifecycle. By default each call is ephemeral and leaves no terminal tab. For a persistent or interactive workflow, set persistent=true, then pass the returned terminal_id to later run_command calls; working_directory applies only when starting a new execution. Risk: runs local commands, respects the command allowlist, and may require user confirmation; timeout_seconds is capped at 300. If unavailable, discover and load it with tool_catalog when the profile permits on-demand tools.`
}
