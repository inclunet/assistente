...
func TestApplyCommandStorageMigrationStampsV20AfterCallback(t *testing.T) {
	...
	if got := commandMigrationStampCount(t, db); got != 1 {
		t.Fatalf("v20 não registrada após callback: %d", got)
	}
	var value string
...
func TestApplyCommandStorageMigrationIsNoopAfterExplicitCompletion(t *testing.T) {
	...
	if calls != 1 {
		t.Fatalf("v20 reaplicada: callback chamado %d vezes", calls)
	}
	if got := commandMigrationStampCount(t, db); got != 1 {
		t.Fatalf("quantidade de carimbos v20 inesperada: %d", got)
	}
}

func TestApplyCommandStorageMigrationRejectsConflictingV20Name(t *testing.T) {
	for _, conflictingName := range []string{"other_storage", ""} {
		t.Run("nome="+conflictingName, func(t *testing.T) {
			...
			if err := db.Exec("INSERT INTO schema_migrations (version, name, applied_at) VALUES (20, ?, CURRENT_TIMESTAMP)", conflictingName).Error; err != nil {
				t.Fatal(err)
			}
			calls := 0
			...
			if err == nil || !strings.Contains(err.Error(), "incompatível") {
				t.Fatalf("colisão de nome v20 não rejeitada: %v", err)
			}
			if calls != 0 {
