...
func (s stubSession) AdminContext() (context.Context, error) {
	return s.adminCtx, s.adminErr
}
...
func TestWithAdminFailClosed(t *testing.T) {
	t.Parallel()
	want := errors.New("not admin")
	_, err := WithAdmin(stubSession{adminErr: want}, func(ctx context.Context) (string, error) {
		t.Fatal("fn não deve rodar sem admin")
		return "", nil
	})
	if !errors.Is(err, want) {
		t.Fatalf("got %v, want %v", err, want)
...
func TestWithAdminOK(t *testing.T) {
	t.Parallel()
	ctx := context.WithValue(context.Background(), ctxUID, "admin")
	got, err := WithAdmin(stubSession{adminCtx: ctx}, func(c context.Context) (string, error) {
		if c.Value(ctxUID) != "admin" {
			t.Fatalf("ctx perdido")
		}
		return "ok", nil
	})
...
func TestWithAdminNilSession(t *testing.T) {
	t.Parallel()
	_, err := WithAdmin[string](nil, func(context.Context) (string, error) {
		t.Fatal("não deve chamar")
		return "", nil
	})
	if !errors.Is(err, errNilSession) {
