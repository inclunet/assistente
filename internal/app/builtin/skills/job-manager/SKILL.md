...
Template examples use the pseudo notation `TPL(expr)` so this skill document is not parsed as a template. When calling the `job` tool, convert `TPL(expr)` to the job runtime's native Go-template delimiters around `expr`. Do not pass `TPL(...)` or HTML entities literally.
...
triggers:                            # required on create (≥ 1)
  ...
  - type: event
    ...
    when: 'TPL(eq .event.type "relevant")'   # optional condition
  - type: manual
...
inputs:
  field: "fixed value"
  dynamic: "TPL(.event.data)"
  secret_val: "TPL(secret \"API_KEY\")"

output:
  map:
    result: "TPL(.output.data)"

events:
  on_success: "my-job.done"
  on_failure: "my-job.failed"
  emit_when: 'TPL(ne .output.status "unchanged")'   # optional
  for_each: "items"                                    # fan-out (optional)
  payload_template: |                                  # reshape payload (optional); wrap every value with `json`
    {
      "id": TPL(json .output.id),
      "source": TPL(json .event.origin)
    }
...
- Context: `TPL(.event.*)` (trigger payload), `TPL(.now)` (current time)
- Empty/omitted = always runs

```yaml
when: 'TPL(eq .event.webhookEvent "jira:issue_updated")'
```
...
- Context: `TPL(.output.*)` (tool result), `TPL(.event.*)` (trigger payload), `TPL(.now)` (current time)
- For fan-out: evaluated **per item** — only matching items emit events
- Empty/omitted = always emits

```yaml
emit_when: 'TPL(eq .output.status "Done")'
```
...
```json
{
  ...
  "triggers": [
    { "type": "event", "listen": "tasklist.list.refresh_requested",
      "when": "TPL(eq .event.task_list_slug \"fsd\")" }
  ],
...
- There is **no `.item`**. In a `for_each` fan-out, the current array element becomes `.output` (see [Fan-out and iteration](#patterns-fan-out-and-iteration)). Writing `TPL(.item.key)` resolves to nothing.
...
|----------|-------------------|-------------|
| `pluck` | `TPL(pluck .output.issues "key")` | Extract a (dot-path) field from every item of a slice → new slice |
| `any` | `TPL(any .output.issues "fields.priority.name" "Critical")` | True if any item's dot-path equals the value |
| `join` | `TPL(join .output.keys ", ")` | Join a slice into a string with a separator |
| `json` | `TPL(json .output)` | Serialize a value to a JSON string (this is the `toJson` replacement) |
| `default` | `TPL(default 50 .event.limit)` | **Argument order is `default <fallback> <value>`** — returns `<value>` unless it is nil/zero, in which case returns `<fallback>`. ⚠️ Fallback comes **first** (unlike Sprig's `default`). |
| `date` | `TPL(date .now "2006-01-02")` | Format a `time.Time` or a date string using a Go layout. Accepts RFC3339 **and** Jira-style ISO-8601 with a numeric `±HHMM` offset (no colon), e.g. `2026-05-25T15:35:53.521-0300`. |
| `jiraTime` | `TPL(jiraTime .output.updated)` | Normalize a Jira-style timestamp (`±HHMM` offset) to an RFC3339 string (`±HH:MM`) that downstream tools accept (e.g. `task_note.external_updated_at`). RFC3339 input passes through. Errors on unparseable input. |
| `now` | `TPL(date (now) "2006-01-02")` | Function returning the current `time.Time`. Takes **no arguments**. ⚠️ Distinct from the root variable `.now` — see the pitfall below. |
| `secret` | `TPL(secret "API_KEY")` | Resolve a secret by name — never hardcode credentials |
| `adf_markdown` | `TPL(adf_markdown .event.description)` | Render an Atlassian Document Format (ADF) node to Markdown |
| `adf_text` | `TPL(adf_text .event.description)` | Render an ADF node to plain text |

### Auto-corrections applied before parsing

Two forgiving rewrites run on every template **before** it is parsed:

- **`fixTemplateDots`**: a leading `TPL(event.x)` / `TPL(output.x)` / `TPL(now)` gets the missing dot → `TPL(.event.x)`. This only fixes the root word at the **start** of a template block; inside `if`/`range`/`with` you must write the dot yourself (e.g. `TPL(if .event.x)`). **Always write the leading dot** — do not rely on the auto-fix.
- **`fixArrayAccess`**: JS-style numeric dot indexing is converted to a Go `index` call → `TPL(.event.content.0.id)` becomes `TPL((index .event.content 0).id)`. You can write either form.

> ⚠️ **`now` function vs `.now` variable pitfall.** Both the `now` *function* (which takes **no arguments** — `func() time.Time`) and the `.now` root *variable* return the current time. But `fixTemplateDots` rewrites a leading `now` without a dot, so any block that **starts** with `now` — `TPL(now)` or even a pipe like `TPL(now | …)` — is silently rewritten to `TPL(.now)`, i.e. it becomes the **variable**, not a function call. (There is no valid `TPL(now "…")` form — passing arguments to `now` is an error anyway.) To actually **call** the function, keep it off the start of the block by **parenthesizing** it: `TPL(date (now) "2006-01-02")` — there the auto-fix leaves it alone. In practice just use the `.now` variable (e.g. `TPL(date .now "2006-01-02")`) unless you specifically need a fresh `time.Now()`.
...
- **Wrap *every* dynamic value with the `json` function — including strings** — and do **not** add manual quotes around it. `json` escapes and quotes the value correctly. Manually quoting (`"id": "TPL(.output.id)"`) does **not** guarantee valid JSON: if the value contains a `"`, a newline or other control characters, `json.Unmarshal` fails and you hit the silent fallback below. Correct form:

  ```
  { "task_code": TPL(json .output.key), "title": TPL(json .output.fields.summary), "raw": TPL(json .output.fields) }
  ```
...
| `template: invalid character ':' in variable reference` | JSON/Jinja-style syntax inside a template block (e.g. `TPL(event:foo)` or `TPL({"a":1})`) | Use Go syntax. To emit a JSON literal in `payload_template`, put the JSON **outside** the template block and only interpolate values inside template blocks |
| `payload_template` seems ignored | Rendered text is not a valid JSON object → silent fallback to raw output | Dry-run and check the emitted payload; render a valid JSON object wrapping **every** dynamic value with `json` (no manual quotes), e.g. `{ "task_code": TPL(json .output.key) }` |
| `date: cannot parse "…-0300"` / whole payload comes out `<no value>` | A **Jira timestamp** (e.g. `2026-05-25T15:35:53.521-0300`) uses a numeric `±HHMM` offset (no colon) → not RFC3339. When `date`/`task_note` rejects it, the template render aborts and you hit the silent fallback (every field empty) | Normalize Jira timestamps with **`jiraTime`** before passing them downstream: `"comment_updated": TPL(json (jiraTime .output.updated))`. (`date` and `task_note.external_updated_at` also accept the `±HHMM` form directly now, but `jiraTime` makes intent explicit.) |
...
| `default` returns the wrong branch | Argument order: it is `default <fallback> <value>` | Put the fallback first: `TPL(default 50 .event.limit)` |
...
2. For each element:
   - If the element is an object, its keys become the event payload (so a child reads `TPL(.event.<key>)`). If it is a scalar, it is wrapped as `TPL(.event.content)`.
   - Two bookkeeping fields are added: `_fan_out_index` and `_fan_out_total`.
   - `emit_when` is evaluated **per item**, with the item exposed as `.output` — filter items with `TPL(.output.X)`.
...
```json
{
  ...
  "events": {
    ...
    "for_each": "issues",
    "emit_when": "TPL(ne .output.fields.status.name \"Done\")",
    "payload_template": "{ \"key\": TPL(json .output.key), \"summary\": TPL(json .output.fields.summary) }"
  }
}
```
...
```json
{
  ...
  "inputs": {
    "task_list_slug": "fsd",
    "title": "TPL(.event.key) — TPL(.event.summary)"
  }
}
```
...
- `_chain_id` — stable id of the whole reactive chain (the originating run id). Read it in a child via `TPL(.event._chain_id)`.
...
when: 'TPL(eq .event._source "user")'
...
when: 'TPL(and (eq .event._source "job") (ne .event._source_job_id "move-card-job"))'
```
...
| Read job | `job(job_id)` | Returns the full job config. Sensitive fields inside the **configured** `inputs` are redacted (by sensitive key names and `TPL(secret ...)` references); `last_run` is cleared. **Naming asymmetry:** the persisted dry-run config is returned under the `dry_run` key (matching `jobs.Job`'s JSON), while create/update accept it as `dry_run_config` — translate the key when round-tripping. |
...
- Use `TPL(.event.*)` in `inputs` to pass data from upstream jobs that emitted the event you are listening to.
- Use `TPL(secret "KEY")` for credentials — never hardcode secrets in `inputs`.
