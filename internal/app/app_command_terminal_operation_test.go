...
func TestCommandTerminalInterruptCatalogAndDeniedWithoutTarget(t *testing.T) {
	...
	d, ok := p.registry.Lookup(commandTerminalInterruptID)
		if !ok || len(p.registry.List()) != 149 || !isWorkspaceMutationCommand(d.ID) || isLocalUICommand(d.ID) || !commandDeckLedgerCommand(d) || !d.HasMutableTarget || d.Persistence.Audit != commandcatalog.PersistenceRedacted || d.Persistence.Arguments != commandcatalog.PersistenceNever || d.Persistence.Result != commandcatalog.PersistenceNever {
