...
func (m *Manager) verifyDEKConsistency(ctx context.Context) error {
	...
	case wrap.DekID == "":
		...
		// abaixo.
		log.Printf("[Credentials] master_wrap sem dek_id — adotando DEK do keychain (%s) como referência", status.KeychainDekID)
	...
	default:
		...
		log.Printf("[Credentials] CRITICAL: divergência DEK detectada — keychain=%s wraps=%s; bloqueando persistência até resolver", status.KeychainDekID, wrap.DekID)
		m.persist = false
		status.OK = false
	...
	if status.HasKeyringDEK {
		...
		if scanErr != nil {
			log.Printf("[Credentials] scan de creds ilegíveis falhou: %v", scanErr)
		}
