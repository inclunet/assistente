...
de que encerra o lifecycle, drena o domínio de executores, fecha a bridge,
remove a configuração volátil do `HostState` e bloqueia novas admissões ou
remontagens no App já fechado. O caminho conservador também permanece coberto:
se o drain de executor falha, o App preserva dependências e não cancela o
contexto de fundo antes de comprovar encerramento. A recuperação pós-drain de
receipts e invocações segue coberta pelos testes dedicados com schema migrado.

Testes novos/combinados cobrem shutdown integrado com bridge/lifecycle/drain,
limpeza do `HostState` no shutdown, preservação de dependências quando drain
falha, recovery de invocações/receipts drenados, shutdown da bridge terminal e
join do monitor de sessão do SO via `bgWG`. Não há cadência paralela nova:
o monitor de SO continua iniciado uma única vez por App e acompanhado pelo
shutdown de background.

**Contagem: 52/84 critérios encerrados; 32 abertos; 3/15 pacotes completos.**
I14.6 segue aberto para a matriz ampla de instalação nova, upgrade, restart com
pendência, falhas em cada etapa e retomada após erro.
...
persistido e limpeza do `HostState` em resets, o runtime produtivo do App agora
revalida a projeção contra o snapshot atual do `HostState` no `Publish` e antes
de habilitar entradas. Se a configuração/camadas/unlock observados mudarem
entre `Project` → `Publish` ou entre `Publish` → `SetEnabled`, a geração antiga
não fica publicada/habilitada. Isso complementa a serialização do controller,
o cancelamento ativo antes de reset, a invalidação de geração no core e os
testes já existentes do monitor de sessão do SO, que limpam mapas em lock,
cancelamento e falha do observador.

Testes novos cobrem projeção stale rejeitada no `Publish`, projeção stale
rejeitada no `SetEnabled`, reset removendo configuração volátil do `HostState`
e publish persistido rejeitado após troca de sessão. A cobertura existente já
provava reset durante readiness, reset/shutdown concorrentes e falha/cancelamento
do monitor apagando mapas somente em memória.

**Contagem: 51/84 critérios encerrados; 33 abertos; 3/15 pacotes completos.**
I14.5–I14.6 seguem abertos para shutdown/manutenção integrados e matriz ampla
de falhas/retomadas.

## Continuação — 15/09/2026, reset remove publicação volátil do HostState em I14.4

Avanço em **I14.4**, ainda sem fechar o subitem. O hook
`resetCommandLifecycleIfConfigured`, usado por login/refresh/logout/troca de
principal, agora também esquece a configuração volátil do usuário atual no
`HostState` antes de resetar o controller. Assim, durante uma transição de auth,
não basta desabilitar a geração do runtime: a projeção que o executor consulta
também deixa de estar publicada para aquela sessão até que um rebuild autenticado
publique uma nova configuração.

Teste novo publica uma configuração no `HostState`, executa o reset de lifecycle
e exige `ErrHostUserNotPublished` na leitura posterior do usuário, provando que
o mapa antigo não fica disponível entre sessões.

**Contagem mantida: 50/84 critérios encerrados; 34 abertos; 3/15 pacotes
completos.** I14.4 ainda fica aberto para a bateria restante de bootstrap
concorrente e falhas de monitor, mas agora tem duas garantias locais: publish
stale rejeitado por sessão e reset removendo a publicação volátil do host.

## Continuação — 15/09/2026, trava de sessão em publicação persistida de I14.4

Avanço em **I14.4**, ainda sem fechar o subitem. A configuração persistida
carregada pelo rebuild produtivo agora carrega junto o principal exato
`userID/sessionID` observado no load. A publicação final rejeita o snapshot se,
entre load/projeção e commit no `HostState`, a sessão local mudar ou o usuário
não corresponder ao escopo carregado. Isso impede que uma reconstrução atrasada
de sessão anterior sobrescreva a configuração volátil da sessão nova.

Teste novo simula troca de sessão entre `loadCommandLifecyclePersistedConfiguration`
e `publish`, exige `ErrDenied` e confirma que a transição stale não é aceita.

**Contagem mantida: 50/84 critérios encerrados; 34 abertos; 3/15 pacotes
completos.** I14.4 permanece aberto até cobrirmos também retomadas concorrentes
mais amplas, logout/troca de usuário durante bootstrap e publicação antiga do
controller/runtime sob falhas de monitor.

## Continuação — 15/09/2026, fechamento qualificado de restart em I14.3

I14.3 foi encerrado localmente. O ciclo login/unlock/restart agora possui um
contrato testado de montagem segura: autenticação monta as dependências reais,
unlock relança o lifecycle após liberar a barreira do cofre, rebuild carrega a
geração persistida quando existe, restaura claims persistentes, deriva camadas
ativas revalidadas e publica no `HostState`; shutdown/drain reconcilia receipts
e invocações somente quando há prova explícita de gerações drenadas. No caminho
de restart de processo, o App volta a carregar/projetar/publicar configuração
válida, mas não infere que invocações pendentes antigas foram drenadas apenas
porque o processo reiniciou. Pendências sem prova permanecem no ledger para
recovery comprovado ou manutenção posterior.

Teste novo cobre esse limite: uma invocação `evaluating` pré-restart não vira
`outcome_unknown` sem prova de drain, enquanto o lifecycle ainda chega a
`Ready` com configuração publicada.

**Contagem: 50/84 critérios encerrados; 34 abertos; 3/15 pacotes completos.**
I14.4–I14.6 seguem abertos para concorrência/publicação antiga, shutdown
integrado completo e matriz ampla de falhas/retomadas.

## Continuação — 15/09/2026, restore de claims persistentes em I14.3

Avanço em **I14.3**, ainda sem fechar o subitem. O rebuild produtivo agora só
roda restore de claims quando já existe geração base de configuração para o
usuário autenticado; em instalação nova continua no sentinel sem alterar claims.
Quando há snapshot persistido, o App usa `commandactivation.RestorePersistent`
com owner/epoch derivados do `HostState`, recarrega o snapshot depois do restore,
deriva `ActiveUserLayerIDs` somente de claims manuais, ativas, persistentes,
da sessão atual, não expiradas, com regra ativa e camada de usuário habilitada,
e publica essa lista junto com a configuração no `HostState`.

Testes novos cobrem a restauração end-to-end de uma claim persistente antiga
até a seleção do binding da camada ativa, além dos filtros que impedem claim
não manual, sessão divergente, expirada, lifecycle não persistente ou camada
desabilitada de ativar bindings.

**Contagem mantida: 49/84 critérios encerrados; 35 abertos; 3/15 pacotes
completos.** I14.3 ainda fica aberto por causa da prova/qualificação de restart
de processo; login, unlock, projeção persistida, recovery pós-drain e claims
persistentes já têm caminho implementado e testado localmente.
...
somente a configuração sentinel: agora tenta carregar o snapshot local de
`commandconfig.Store` para o usuário autenticado, diferencia instalação ainda
sem geração base de erro real, projeta o subconjunto `keyboard.local`/read/none
via `ProjectLocalRead`, revalida a geração com `Store.CheckCurrent` dentro da
publicação e só então publica no `HostState`. Quando não existe geração
persistida, mantém o fallback sentinel fail-closed anterior; quando existe
geração inválida/obsoleta, a falha permanece observável e não é mascarada como
mapa vazio. O teste novo cobre um delta persistido sobre default builtin do
lifecycle sendo restaurado após auth, sem restaurar claims de camada de usuário.

**Contagem mantida: 49/84 critérios encerrados; 35 abertos; 3/15 pacotes
completos.** I14.3 segue aberto porque ainda faltam recovery/reconciliação de
restart, receipts/invocações/claims pendentes e integração explícita do unlock.

Validação: `go test ./internal/app -run "TestAppCommandLifecycle" -count=1`
passou. Validação ampla será repetida ao fechar o bloco.
...
montagem produtiva mínima pós-auth: `ensureCommandLifecycleMountedForCurrentUser`
cria `HostState`, store ledger real sobre `database.DB()`, catálogo completo
sentinel `lifecycle.ready`, bridge real com adapter interno fail-closed,
`FactBus` real via providers do App e portas padrão de `commandruntime`.
`bootstrapCommandLifecycleAfterAuth` tenta essa montagem quando ainda não há
lifecycle; falha de dependência não quebra Login/RefreshAuth e mantém comandos
fail-closed. A montagem instala `HostState`/bridge reais e só aceita instâncias
coerentes. O sentinel não migra comandos antigos nem expõe ação de produto: ele
serve para provar catálogo/defaults/políticas/stores/presenter/providers/
dispatcher/adapters sem fallback permissivo.

**Contagem: 49/84 critérios encerrados; 35 abertos; 3/15 pacotes completos.**
I14.3 permanece aberto para orquestrar rebuild/recovery/projeção efetiva depois
de login/unlock/restart; I14.4–I14.6 seguem abertos.
...
**Contagem histórica: 48/84 critérios encerrados; 36 abertos; 3/15 pacotes completos.**
I13 foi consolidado como terceiro pacote completo após os testes físicos de
Stream Deck e ambiente físico. Este limite foi superado pelo fechamento de
I14.2 registrado acima.
...
Evidência/limite atual: `commandruntime.Controller` serializa bootstrap, invalida/cancela antes do reset e confirma readiness via core compartilhado; exige todas as portas e recusa projeção vazia. `MountSpec` declara dependências obrigatórias de catálogo, defaults, políticas, stores, presenter, providers, dispatcher e adapters; `NewMounted` recusa manifesto incompleto, duplicado ou nil antes de criar controller. `App.commandLifecycle` é um ponteiro atômico privado, com montagem serializada, barreira terminal de shutdown e desmontagem por CAS, sem registry global. `ConfigureCommandLifecycleMountSpec` liga o manifesto ao App sem instalar runtime quando o contrato falha; `ConfigureCommandLifecycleForApp` preenche esse manifesto com `commandexecution.Config`, `HostState`, `commandbridge.Bridge`, `commandcontext.FactBus`, presenter real de decisão e adapter fornecido pelo host, instala o `HostState` e a bridge reais, e recusa storage sem versão, presenter ausente, adapter nil, bridge divergente e `HostState` de outro epoch. `ensureCommandLifecycleMountedForCurrentUser` cria a montagem produtiva mínima pós-auth com store ledger real, catálogo completo sentinel, providers reais e adapter interno fail-closed; `bootstrapCommandLifecycleAfterAuth` tenta montar quando necessário sem quebrar Login/RefreshAuth. O pós-auth já carrega `commandconfig.Store` quando há geração base, restaura claims persistentes com `commandactivation.RestorePersistent`, deriva `ActiveUserLayerIDs` de claims manuais/persistentes atuais e projeta `keyboard.local`/read/none com `ProjectLocalRead`; sem geração persistida, usa sentinel sem alterar claims. A publicação persistida fixa o principal `userID/sessionID` capturado no load e rejeita commit se a sessão mudou antes da publicação, impedindo overwrite stale. Resets de login/refresh/logout agora também removem a configuração volátil do usuário atual no `HostState`, impedindo que mapa antigo continue consultável entre sessões. O runtime produtivo revalida a projeção contra o `HostState` atual no `Publish` e no `SetEnabled`, recusando configuração/camadas/unlock antigos entre as etapas. `SetupVault`/`UnlockVault` relançam bootstrap após liberar a barreira de auth, e `drainCommandExecutors` usa prova de `CloseAndDrain` para recuperar receipts e invocações pendentes de gerações drenadas antes de destruir dependências. Shutdown integrado encerra lifecycle, drena executores, fecha bridge, limpa `HostState`, bloqueia novas admissões/remontagens e preserva dependências quando drain falha. Restart de processo é qualificado explicitamente: reabre carregando/projetando/publicando configuração válida, mas não reconcilia invocações/receipts pendentes por inferência sem prova de drain. Os testes cobrem falhas/cancelamento, configurações concorrentes, montagem incompleta, bootstrap pronto com portas reais, montagem pós-auth, rebuild sentinel/persistido, restore de claim persistente, filtros de ativação, rejeição de publicação stale após troca de sessão, reset/shutdown removendo publicação do HostState, projeção stale recusada em publish/enable, bootstrap pós-unlock, recovery pós-drain, bridge/lifecycle/drain em shutdown integrado, falha/cancelamento do monitor de SO e restart sem falsa reconciliação.
