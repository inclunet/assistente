...
A exclusão de conversas, inclusive em lote, compartilha um gate cancelável com a compactação. Assim, `VACUUM` e a transação destrutiva `BEGIN IMMEDIATE` nunca disputam o arquivo entre si; writers de dados associados validam a conversa sob o mesmo writer lock e usam o retry canônico de `SQLITE_BUSY`, impedindo recriação pós-delete. Conversas com run de subagente, entrega terminal ao pai, pipeline/stream LLM ou callback de canal reservado/ativo são recusadas sem cancelar o trabalho antes do commit. O write gate dos subagentes permanece fechado até o desfecho da transação. Em sucesso, os gates de stream/notifier são convertidos em tombstones por conversa no commit; chamadas bloqueadas são então recusadas, enquanto os mutexes globais são liberados antes da limpeza pós-commit. Em rollback, nenhum tombstone permanece. A ordem de aquisição antecede o lock SQLite e é estável, evitando deadlock. Evidências: `internal/database/conversation_batch_delete_test.go`, `internal/subagent/manager_test.go`, `internal/chat/streaming_manager_test.go` e `internal/messaging/notifier_test.go`.
