...
| **Ctrl+H** | Abre o histórico do painel de chat ativo |
| **Ctrl+P** | Abre o seletor de perfil de interação do chat ativo |
| **Ctrl+L** | Limpa a conversa do painel de chat ativo |

Os quatro atalhos são contextuais à surface de chat ativa. Não são capturados
quando o foco pertence a Monaco, campo editável, terminal, modal ou diálogo
incompatível, menu, picker já aberto ou outra surface; repetições de tecla são
ignoradas. O chat modal é a exceção entre os modais: quando estiver aberto,
somente a toolbar desse modal, se ele estiver no topo, responde aos atalhos.
