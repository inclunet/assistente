...
2. Verificar se existe usuário admin local.
   - Se não existir, abrir diretamente a etapa **Criar Admin Local** do wizard, sem recriar cofre nem recovery key; ao salvar o admin, executar a migração/backfill idempotente de dados legados, emitir sessão local/refresh token e seguir para onboarding pós-login.
