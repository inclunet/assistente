// @ts-check
// Cynhyrchwyd y ffeil hon yn awtomatig. PEIDIWCH Â MODIWL
// This file is automatically generated. DO NOT EDIT

export function GetTaskListCustomActions(arg1) {
  return window['go']['wailsapi']['TasklistActions']['GetTaskListCustomActions'](arg1);
}

export function ListBoardCustomActions(arg1) {
  return window['go']['wailsapi']['TasklistActions']['ListBoardCustomActions'](arg1);
}

export function ListCardCustomActions(arg1, arg2) {
  return window['go']['wailsapi']['TasklistActions']['ListCardCustomActions'](arg1, arg2);
}

export function SetTaskListCustomActions(arg1, arg2) {
  return window['go']['wailsapi']['TasklistActions']['SetTaskListCustomActions'](arg1, arg2);
}

export function TriggerCustomAction(arg1, arg2, arg3) {
  return window['go']['wailsapi']['TasklistActions']['TriggerCustomAction'](arg1, arg2, arg3);
}
