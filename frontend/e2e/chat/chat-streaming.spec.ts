...
test.describe('Chat — erro no envio', () => {
  test('exibe mensagem de erro quando SendMessage falha', async ({ page, wails }) => {
    ...
    const errorMessage = page.locator('.ant-alert, .chat-message').filter({ hasText: /Network error/ });
  ...
  test('input é reabilitado após erro de envio', async ({ page, wails }) => {
    ...
    await page.locator('.ant-alert, .chat-message').filter({ hasText: /Timeout/ }).waitFor({ timeout: 5_000 });
  ...
  test('mensagem de erro contém detalhes do erro original', async ({ page, wails }) => {
    ...
    const errorMessage = page.locator('.ant-alert, .chat-message').filter({ hasText: 'Connection refused' });
    await expect(errorMessage).toBeVisible({ timeout: 5_000 });
  });
