const en = {
  translation: {
    ...
    profiles: {
      ...
      nativeMcpAuto: 'Automatic (adapter)',
      nativeMcpOn: 'Force native',
      nativeMcpOff: 'Force adapter (function)',
      nativeMcpHint: "How MCP servers are delivered to the provider. Native mode uses the provider's format (OpenAI Responses → type:mcp; Anthropic → mcp_servers). Automatic = adapter mode (function) by default, compatible with any model. 'Force native' only takes effect on physically capable providers (Responses/Anthropic).",
