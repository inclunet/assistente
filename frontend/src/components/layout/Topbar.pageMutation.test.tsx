...
import { usePageMutationCommands, type PageMutationID, type PageMutationOutcome, type PageMutationRequest } from '../../lib/commandPageMutation';

const state = vi.hoisted(() => ({
  commandId: 'tasklists.update' as PageMutationID,
  pathname: '/tasklists',
  ready: false,
  pageAvailable: true,
  flush: vi.fn(),
...
vi.mock('../../services/commandCatalog', () => ({
  listCommandCatalog: vi.fn(async () => [{
    ...
    description: 'Atualiza a lista selecionada',
    category: 'tasklists',
    aliases: ['atualizar'],
    risk: 'write',
    available: true,
...
function PageMutationSurface({ onNativeOutcome }: { onNativeOutcome: (outcome: PageMutationOutcome) => void }) {
  const root = useRef<HTMLDivElement>(null);
  const { request: requestMutation } = usePageMutationCommands({
    root,
    pathname: state.pathname,
    allowedCommands: [state.commandId],
  ...
  return (
    <div ref={root}>
      ...
      <button type="button" onClick={() => { void requestMutation(state.commandId).then(onNativeOutcome); }}>
...
let nativeOutcome: PageMutationOutcome | undefined;

beforeEach(() => {
  state.commandId = 'tasklists.update';
  reservation.commandId = state.commandId;
  handoff.commandId = state.commandId;
  ...
  state.begin.mockResolvedValue(reservation);
  state.beginKey.mockResolvedValue(reservation);
  state.prepare.mockResolvedValue(undefined);
  state.take.mockResolvedValue(handoff);
...
async function mount() {
  state.pathname = '/tasklists';
  render(<><Topbar /><PageMutationSurface onNativeOutcome={outcome => { nativeOutcome = outcome; }} /></>);
...
async function trigger(source: 'native' | 'palette' | 'keyboard' | 'deck') {
  ...
  if (source === 'palette') {
    ...
    await user.click(await screen.findByRole('option', { name: /Atualizar lista/ }));
    return;
  }
  if (source === 'keyboard') {
  ...
  act(() => { state.events.get('command:deck-ui-reservation')?.(reservation); });
}

async function expectCommitted() {
  await waitFor(() => expect(state.pageResult).toHaveBeenCalledOnce());
  expect(state.prepare).toHaveBeenCalledExactlyOnceWith('ticket-a', request);
...
it.each(['native', 'palette', 'keyboard', 'deck'] as const)('executa a mutação por %s no pipeline backend compartilhado', async source => {
  await mount();
  await trigger(source);
  await expectCommitted();
  if (source === 'keyboard') {
    expect(state.beginKey).toHaveBeenCalledExactlyOnceWith(
      'generation-a',
      { version: 1, code: 'KeyU', modifiers: ['Control'] },
      false,
    );
    ...
    expect(state.begin).toHaveBeenCalledExactlyOnceWith('tasklists.update');
  }
});

it('mantém a captura da origem ao selecionar pela paleta', async () => {
  await mount();
  const user = userEvent.setup();
  await user.click(screen.getByRole('button', { name: 'commandPalette.title' }));
  const search = await screen.findByRole('combobox');
  await user.type(search, 'Atualizar lista');
  await user.click(await screen.findByRole('option', { name: /Atualizar lista/ }));
  await expectCommitted();
});

it.each(['native', 'palette', 'keyboard', 'deck'] as const)('limpar lista por %s usa o commit backend, não o writer nativo', async source => {
  state.commandId = 'tasklists.clear';
  reservation.commandId = state.commandId;
  handoff.commandId = state.commandId;
  await mount();
  await trigger(source);
  await expectCommitted();
  if (source === 'native' || source === 'palette') expect(state.begin).toHaveBeenCalledWith('tasklists.clear');
});

it('cancela a reserva do Deck quando a captura da página fica indisponível', async () => {
  await mount();
...
it('recusa a execução quando a preparação backend falha', async () => {
  await mount();
  state.prepare.mockRejectedValueOnce(new Error('fingerprint stale'));
  await trigger('native');
  await waitFor(() => expect(state.cancel).toHaveBeenCalledWith('ticket-a'));
  await waitFor(() => expect(nativeOutcome?.status).toBe('failed'));
  expect(state.commit).not.toHaveBeenCalled();
