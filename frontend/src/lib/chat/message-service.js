...
async function loadWailsFunctions() {
  ...
  _wailsLoadPromise = (async () => {
    try {
      ...
      // Silenciado: funções Wails não disponíveis
    }
    ...
    try {
      ...
      // Silenciado: runtime Wails não disponível
    }
...
export function convertMessageNode(node, index = 0, level = 0) {
  ...
  const children = rawChildren.map((child, childIndex) => convertMessageNode(child, childIndex, (node.level ?? node.Level ?? level) + 1));
  ...
  return {
    ...
    message: {
      ...
      toolName: toolName,
      internal: (m.parent_id || m.ParentID) != null || m.internal || m.Internal || false,
    ...
    children,
    childCount: node.child_count ?? node.ChildCount ?? node.childCount ?? children.length
  };
}
...
class MessageService extends EventTarget {
  ...
  _normalizeThreads(threads, level = 0) {
    return (threads || []).map((node, index) => {
      if (node?.__normalized) {
        const nodeLevel = node.level ?? level;
        const normalizedChildren = this._normalizeThreads(node.children, nodeLevel + 1);
        return {
          ...node,
          level: nodeLevel,
          originalIndex: node.originalIndex ?? index,
  ...
  _applyThreads(rawThreads, extraData = null) {
    const normalized = this._normalizeThreads(rawThreads);
    ...
    this._stores.conversationData.set(updatedData);
    this._stores.threadedMessages.set(normalized);
    this._stores.messages.set(this._extractMessagesFromThreads(normalized));
    return normalized;
  }
   ...
   */
  bindBackendEvents() {
    console.log(`[MessageService ${this._instanceId || 'singleton'}] Vinculando eventos backend`);
    
    if (!EventsOn) {
      console.warn(`[MessageService ${this._instanceId || 'singleton'}] EventsOn não disponível`);
      return;
    }
    
    // Se já está bound, faz unbind primeiro (suporta hot reload)
    if (this._boundToBackend) {
      console.log(`[MessageService ${this._instanceId || 'singleton'}] Já estava bound, fazendo unbind primeiro`);
   ...
   */
  unbindBackendEvents() {
    console.log(`[MessageService ${this._instanceId || 'singleton'}] Desvinculando eventos backend`);
    if (EventsOff) {
   ...
   */
  destroy() {
    console.log(`[MessageService ${this._instanceId || 'singleton'}] Destruindo instância`);
    this.unbindBackendEvents();
  ...
  _isEventForThisInstance(eventConversationId) {
    ...
    if (!eventConversationId) {
      console.log(`[MessageService ${this._instanceId || 'singleton'}] Evento sem conversationId - ACEITO`);
      return true;
    }
    
    const currentConvId = get(this._stores.conversationId);
    if (!currentConvId) {
      console.log(`[MessageService ${this._instanceId || 'singleton'}] Sem conversa local, evento conversationId=${eventConversationId} - ACEITO`);
      return true; // Nova conversa, aceita
    }
    
    const matches = eventConversationId === currentConvId;
    if (!matches) {
      console.log(`[MessageService ${this._instanceId || 'singleton'}] Evento REJEITADO - eventConvId=${eventConversationId}, currentConvId=${currentConvId}`);
    } else {
      console.log(`[MessageService ${this._instanceId || 'singleton'}] Evento ACEITO - conversationId=${currentConvId}`);
    }
    return matches;
  }
  ...
  _handleStreamEvent(event) {
    ...
    console.log(`[MessageService ${this._instanceId || 'singleton'}] _handleStreamEvent recebido - conversationId=${eventConvId}`);
    
    if (!this._isEventForThisInstance(eventConvId)) {
      console.log(`[MessageService ${this._instanceId || 'singleton'}] _handleStreamEvent IGNORADO`);
      return;
    }
    
    console.log(`[MessageService ${this._instanceId || 'singleton'}] _handleStreamEvent PROCESSANDO`);
    ...
    console.log(`[MessageService ${this._instanceId || 'singleton'}] Stream data:`, {
      messageId,
      contentLength: content?.length || 0,
      done,
      hasStreamingMessageId: !!get(this._stores.streamingMessageId)
    });
    
    if (messageId && !get(this._stores.streamingMessageId)) {
      // Inicia streaming com ID do backend
      console.log(`[MessageService ${this._instanceId || 'singleton'}] Iniciando streaming com messageId=${messageId}`);
      this._stores.streamingMessageId.set(messageId);
      this._stores.isStreaming.set(true);
      this._addStreamingPlaceholder(); // Adiciona placeholder na UI
      this._dispatchEvent('streamingStarted', { messageId });
    }
    
    if (content && !done) {
      console.log(`[MessageService ${this._instanceId || 'singleton'}] Atualizando conteúdo do streaming: ${content.substring(0, 50)}...`);
      
      // Garante que existe placeholder (caso o messageId não tenha vindo no primeiro evento)
      if (!get(this._stores.streamingMessageId)) {
        console.log(`[MessageService ${this._instanceId || 'singleton'}] Criando placeholder sem messageId`);
        this._stores.isStreaming.set(true);
    ...
    if (done) {
      console.log(`[MessageService ${this._instanceId || 'singleton'}] Streaming FINALIZADO`);
      
      // Streaming finalizado
      const fullResponse = event.FullResponse ?? event.fullResponse ?? get(this._stores.streamingContent);
      
      console.log(`[MessageService ${this._instanceId || 'singleton'}] Resposta completa: ${fullResponse.substring(0, 50)}...`);
      ...
      console.log(`[MessageService ${this._instanceId || 'singleton'}] Recarregando conversa do banco...`);
  ...
  _handleToolsExecution(data) {
    ...
    // Atualiza último node do assistente (geralmente placeholder em streaming) para mostrar tools
    const conv = get(this._stores.conversationData);
    if (conv?.threads && conv.threads.length > 0) {
      const idx = [...conv.threads].reverse().findIndex(t => t.message?.role === 'assistant');
      if (idx >= 0) {
        const realIdx = conv.threads.length - 1 - idx;
        const newThreads = [...conv.threads];
        newThreads[realIdx] = {
          ...newThreads[realIdx],
          message: { ...newThreads[realIdx].message, toolsInfo: `🔧 ${get(this._stores.toolsMessage)}` }
        };
        this._applyThreads(newThreads);
      }
    }
  ...
  _handleConversationCreated(data) {
    ...
    const currentConvId = get(this._stores.conversationId);
    console.log(`[MessageService ${this._instanceId || 'singleton'}] _handleConversationCreated - currentConvId=${currentConvId}, data.id=${data.id}`);
    
    if (currentConvId !== null) {
      console.log(`[MessageService ${this._instanceId || 'singleton'}] _handleConversationCreated IGNORADO - já tem conversa`);
      return; // Já tem conversa, ignora
    }
    
    // Backend envia 'id', não 'conversationId'
    const conversationId = data.id ?? data.conversationId;
    
    console.log(`[MessageService ${this._instanceId || 'singleton'}] _handleConversationCreated SETANDO conversationId=${conversationId}`);
  ...
  async _handleMessagesReady(data) {
    // Verifica se o evento pertence a esta instância
    const eventConvId = data.conversationId ?? data.ConversationId;
    console.log(`[MessageService ${this._instanceId || 'singleton'}] _handleMessagesReady - conversationId=${eventConvId}`);
    
    if (!this._isEventForThisInstance(eventConvId)) {
      console.log(`[MessageService ${this._instanceId || 'singleton'}] _handleMessagesReady IGNORADO`);
      return;
    }
    
    console.log(`[MessageService ${this._instanceId || 'singleton'}] _handleMessagesReady PROCESSANDO`);
  ...
  _handleInternalMessage(data) {
    ...
    const conversationSnapshot = get(this._stores.conversationData);
    if (parentId && conversationSnapshot?.threads) {
      const parentNode = this._findNodeById(conversationSnapshot.threads, parentId);
      if (parentNode) {
        // Cria node filho
        const childNode = {
          message: internalMsg,
        ...
        this._applyThreads(conversationSnapshot.threads);
      }
    }
  ...
  _addStreamingPlaceholder() {
    console.log(`[MessageService ${this._instanceId || 'singleton'}] _addStreamingPlaceholder chamado`);
    ...
    if (existingIdx >= 0) {
      console.log(`[MessageService ${this._instanceId || 'singleton'}] Placeholder já existe no índice ${existingIdx}`);
      // Já existe, não adiciona outro
      return;
    }
    
    console.log(`[MessageService ${this._instanceId || 'singleton'}] Criando novo placeholder`);
    ...
    console.log(`[MessageService ${this._instanceId || 'singleton'}] Aplicando threads com placeholder (total: ${newThreads.length})`);
    this._applyThreads(newThreads);
    
    console.log(`[MessageService ${this._instanceId || 'singleton'}] Disparando messagesUpdated`);
    this._dispatchEvent('messagesUpdated', { 
      messages: get(this._stores.messages),
      threads: newThreads
    });
  ...
  _updateStreamingMessage(content, isStreamingFlag) {
    console.log(`[MessageService ${this._instanceId || 'singleton'}] _updateStreamingMessage - contentLength=${content?.length}, isStreaming=${isStreamingFlag}`);
    ...
    console.log(`[MessageService ${this._instanceId || 'singleton'}] Placeholder no array flat: índice=${idx}`);
    ...
    console.log(`[MessageService ${this._instanceId || 'singleton'}] Placeholder nos threads: índice=${threadIdx}`);
    
    // Atualiza nos threads e aplica snapshot
    if (threadIdx >= 0 && get(this._stores.conversationData)?.threads) {
      ...
      console.log(`[MessageService ${this._instanceId || 'singleton'}] Atualizando thread no índice ${threadIdx}`);
      this._applyThreads(newThreads);
    } else {
      console.warn(`[MessageService ${this._instanceId || 'singleton'}] Thread de streaming NÃO encontrado!`);
    }
  ...
  _removeStreamingMessage() {
    // Remove dos threads e aplica snapshot
    if (get(this._stores.conversationData)?.threads) {
      const currentData = get(this._stores.conversationData);
  ...
  async loadConversation(conversation, defaultModel = '') {
    ...
    try {
      ...
      // Aplica threads e deriva mensagens exclusivamente dos threads
      this._applyThreads(rootMessages, {
        id: convInfo.id,
        title: convInfo.title,
        model: conversationData.model,
        show_internal_messages: conversationData.show_internal_messages,
        preferences
      });
  ...
  /**
   * Define o conversationId da conversa
   * Usado quando o backend retorna o ID após criar uma nova conversa
   */
  setConversationId(conversationId) {
    this._stores.conversationId.set(conversationId);
    console.log('[MessageService] ConversationId atualizado para:', conversationId);
  }
  ...
  updateMessageContent(messageId, content) {
    // Atualiza nos threads
    const conversationSnapshot = get(this._stores.conversationData);
    if (conversationSnapshot?.threads) {
      const updated = this._updateInThreads(conversationSnapshot.threads, messageId, content);
      if (updated) {
        this._applyThreads(conversationSnapshot.threads);
      }
    }
  ...
  async reload() {
    ...
    // Remove placeholders de streaming antigos antes de recarregar
    this._removeStreamingMessage();
    ...
    try {
      ...
      // Aplica threads unificados preservando filhos já carregados
      this._applyThreads(mergedThreads);
  ...
  _extractMessagesFromThreads(threads) {
    ...
    const traverse = (nodes) => {
      for (const node of nodes) {
        ...
        // Parseia tool_calls
        let toolCalls = null;
        const rawToolCalls = m.toolCalls || m.tool_calls || m.ToolCalls;
        if (rawToolCalls) {
          try {
            toolCalls = typeof rawToolCalls === 'string' ? JSON.parse(rawToolCalls) : rawToolCalls;
          } catch (e) { /* ignore */ }
        ...
        if (m.media) {
          try {
            const mediaSource = typeof m.media === 'string' ? JSON.parse(m.media) : m.media;
        ...
        const parentId = m.parentId ?? m.parent_id ?? m.ParentID ?? null;
        const toolCallId = m.toolCallId ?? m.tool_call_id ?? m.ToolCallID ?? '';
        const agentName = m.agentName ?? m.agent_name ?? m.AgentName ?? '';
        const internal = (parentId != null) || m.internal || m.Internal || false;
        ...
        result.push({
          ...
          internal,
          media,
          // Propaga estado de streaming para o array flat
          isStreaming: m.isStreaming || m.IsStreaming || node.isStreaming || false,
          level: node.level ?? 0
        });
  ...
  _dispatchEvent(type, detail) {
    if (type === 'messagesUpdated') {
      console.log(`[MessageService ${this._instanceId || 'singleton'}] _dispatchEvent(${type})`, {
        messagesCount: detail?.messages?.length,
        threadsCount: detail?.threads?.length,
        listenerCount: this._eventListeners?.get(type)?.size || 0
      });
    }
